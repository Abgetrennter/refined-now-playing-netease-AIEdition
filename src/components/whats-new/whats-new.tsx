								...
								<ul>
									{
										version.changes.map((change, index) => (
											<li className={`${change[2] ? 'major' : ''}`} key={index}>
												{
													(() => {
														switch (change[0]) {
															case 'add':
...
export function whatsNew(force = false) {
	...
	if ((changeLog as ChangeLogItem[]).filter(version => compareVersions(version.version, lastVersion) > 0).length === 0 && !force) return;
	const whatsNewDiv = document.createElement("div");
	whatsNewDiv.id = "refined-now-playing-whats-new";
	document.body.appendChild(whatsNewDiv);
	if (force) {
		lastVersion = "0.0.0";
	}
	// @ts-ignore
	const root = ReactDOM.createRoot(whatsNewDiv);
	root.render(<WhatsNew lastVersion={lastVersion} />);
}
