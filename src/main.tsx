 ...
 */
const updateAccentColor = (name: string, argb: number, isFM: boolean = false) => {
...
let lastDom: HTMLImageElement | null = null, lastIsFM: boolean = false;
/**
 * 计算并应用主题色
 ...
 * @param {boolean} isFM - 是否为私人FM模式 / Is FM mode
 */
const calcAccentColor = (dom: HTMLImageElement, isFM: boolean = false) => {
    ...
    updateAccentColor('rnp-accent-color-dark', (theme.schemes.dark as any)[variant], isFM);
 ...
 */
const recalculateTitleSize = (forceRefresh: boolean = false) => {
...
const addOrRemoveGlobalClassByOption = (className: string, optionValue: boolean) => {
    if (optionValue) {
 ...
 * @param {boolean} isFM - 是否为私人FM模式 / Is FM mode
 */
const addSettingsMenu = async (isFM: boolean = false) => {
     ...
     */
    const bindCheckboxToClass = (checkbox: HTMLInputElement, className: string, defaultValue: boolean = false, callback: (checked: boolean) => void = () => {}) => {
     ...
     * @param {boolean} defaultValue - 默认值
     */
    const bindCheckboxToFunction = (checkbox: HTMLInputElement, func: (checked: boolean) => void, defaultValue: boolean = false) => {
        checkbox.checked = getSetting(checkbox.id, defaultValue);
        checkbox.addEventListener("change", (e: any) => {
            shouldSettingMenuReload[isFM ? 1 : 0] = true;
            setSetting(checkbox.id, e.target.checked);
     ...
     */
    const bindSliderToCSSVariable = (slider: HTMLInputElement, variable: string, defaultValue: number = 0, event: string = 'input', mapping: (val: any) => any = (x) => { return x }, addClassWhenAdjusting: string = '') => {
     ...
     * @param {string} addClassWhenAdjusting - 调整时添加的类名
     */
    const bindSliderToFunction = (slider: HTMLInputElement, func: (val: any) => void, defaultValue: number = 0, event: string = 'input', mapping: (val: any) => any = (x) => { return x }, addClassWhenAdjusting: string = '') => {
        slider.value = getSetting(slider.id, defaultValue);
        slider.dispatchEvent(new Event("input"));
        slider.addEventListener(event, (e: any) => {
            const value = e.target.value;
     ...
     * @param {function} callback - 回调函数
     */
    const bindSelectGroupToClasses = (selectGroup: HTMLElement, defaultValue: string, mapping: (val: string) => string = (x) => { return x }, callback: (val: string) => void = (x) => {}) => {
    ...
    const initSettings = () => {
        ...
        bindSliderToCSSVariable(fluidBlur, '--fluid-blur', 6, 'change', (x) => `${parseInt(Math.pow(2, x) as any)}px`);
 ...
 */
const addFullScreenButton = (container: Element | null) => {
    if (!container) return;
